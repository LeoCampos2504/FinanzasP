...
export async function GET() {
  ...
  try {
    ...
    const accounts = (result.results || []).filter(isActiveNotionPage).map((p: any) => ({ id: p.id, name: getTitle(p), balance: getFormulaNumber(p, "Saldo esperado") || getNumber(p, "Saldo inicial"), primary: getCheckbox(p, "Es caja principal") }));
