...
import { demoAccounts, demoDebtors, demoMovements } from "@/lib/demo-data";
...
export async function GET() {
  ...
  if (isDemoMode() || !getEnv("MOVIMIENTOS_DATA_SOURCE_ID")) return NextResponse.json({ ok: true, data: buildDashboard(demoAccounts, demoMovements, demoDebtors), meta: { demo: true } });
  try {
    const [accountsResult, movementsResult, debtorsResult] = await Promise.all([
    ...
    return NextResponse.json({ ok: true, data: buildDashboard(accounts, movements, debtors) });
...
function buildDashboard(accounts: Account[], movements: Movement[], debtors: any[]) {
