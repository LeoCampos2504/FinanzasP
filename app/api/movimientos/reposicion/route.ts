...
export async function POST(request: Request) {
  ...
  catch (error) { const isProduct = error instanceof ProductOperationError; const isSchema = error instanceof SchemaValidationError; const code = isProduct ? error.code : isSchema ? error.code : "NOTION_ERROR"; const status = code === "VALIDATION" || code === "VARIANT_NOT_FOUND" ? 400 : code === "CONFIG_MISSING" ? 503 : code === "NOTION_SCHEMA_MISSING_PROPERTY" ? 422 : 502; const message = isProduct || isSchema ? error.message : formatNotionError(error, "No se pudo guardar la reposición.", "Movimientos / Detalle de productos"); return NextResponse.json({ ok: false, error: { code, message, details: isProduct ? error.details : undefined } }, { status }); }
}
