...
async function attachOpenCashRegister(input: ProductSaleInput, session: PermissionSession) { try { const caja = await getOpenCashRegisterForSession(session); return caja ? { ...input, cashRegisterId: caja.id } : input; } catch { return input; } }
