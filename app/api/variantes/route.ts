...
import { canAccessBusiness, canManageProducts, isGlobalAdmin } from "@/lib/permissions";
import { listDemoVariants } from "@/lib/demo-replenishment-store";
...
export async function GET(request: Request) {
  ...
  if (isDemoMode()) return NextResponse.json({ ok: true, data: filterVariants(listDemoVariants().filter((variant) => isGlobalAdmin(session) || !variant.businessId || canAccessBusiness(session, variant.businessId)), { search, stockStatus, productBaseId, includeInactive }), meta: { demo: true } });
