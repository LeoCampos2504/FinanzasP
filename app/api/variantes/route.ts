...
export async function POST(request: Request) {
  ...
  const dataSourceId = getEnv("VARIANTES_DATA_SOURCE_ID"); if (!dataSourceId) return NextResponse.json({ ok: false, error: { code: "CONFIG_MISSING", message: "Falta configurar VARIANTES_DATA_SOURCE_ID." } }, { status: 503 });
  try { const schema = await getDataSourceSchema(dataSourceId); const businessId = await resolveBusinessId(); const built = buildVariantProperties(schema, input, businessId); const page = await createPage(dataSourceId, built.properties); return NextResponse.json({ ok: true, data: { id: page.id, name: input.name }, meta: { warnings: built.warnings } }); }
