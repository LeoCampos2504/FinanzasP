...
export async function PATCH(request: Request, { params }: { params: Promise<{ productId: string }> }) {
  ...
  try { const schema = await getDataSourceSchema(dataSourceId); const businessId = input.name !== undefined ? await resolveBusinessId() : ""; const built = buildProductProperties(schema, input, businessId, false); const page = await updatePage(productId, built.properties); return NextResponse.json({ ok: true, data: mapProductBase(page), meta: { warnings: built.warnings } }); }
