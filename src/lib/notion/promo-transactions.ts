...
export async function createPromoSale(input: PromoSaleInput) {
  ...
  const total = calculatePromoTotal(input.mode, context?.promo, items, input.manualTotal, input.quantity);
  ...
  const movementBuilt = buildSchemaAwareProperties(movementSchema, "Movimientos", {
    name: { candidates: ["Nombre"], value: title(`Venta promo - ${promoName}`), required: true }, date: { candidates: ["Fecha"], value: date(input.date), required: true }, scope: { candidates: ["Ámbito"], value: select("Negocio"), required: true }, business: { candidates: ["Negocio"], value: businessId ? relation(businessId) : undefined, required: Boolean(businessId), label: "Negocio" }, type: { candidates: ["Tipo"], value: select("Ingreso"), required: true }, subtype: { candidates: ["Subtipo"], value: select(input.mode === "fixed" ? "Venta promo fija" : "Venta promo personalizada"), required: true }, account: { candidates: ["Cuenta"], value: relation(input.accountId), required: true }, amount: { candidates: ["Monto"], value: number(total), required: true }, paymentStatus: { candidates: ["Estado de pago", "Estado"], value: select("Pagado"), required: true, label: "Estado de pago" }, moneyOrigin: { candidates: ["Origen del dinero"], value: select("No aplica"), required: true }, description: { candidates: ["Descripción", "Descripcion", "Notas", "Nota"], value: input.description ? richText(input.description) : undefined, label: "Descripción" }, active: { candidates: ["Activo"], value: checkbox(true), required: true }, accountEntry: { candidates: ["Entrada cuenta"], value: number(total), label: "Entrada cuenta" }, accountNet: { candidates: ["Movimiento neto cuenta"], value: number(total), label: "Movimiento neto cuenta" }, cashRegister: { candidates: movementCashRegisterCandidates, value: input.cashRegisterId ? relation(input.cashRegisterId) : undefined, label: "Caja" }, user: { candidates: movementUserCandidates, value: input.userId ? relation(input.userId) : undefined, label: "Realizado por" }, paymentReceived: { candidates: movementPaymentReceivedCandidates, value: input.payment?.isCash && input.payment.received !== undefined && input.payment.received !== null ? number(input.payment.received) : undefined, label: "Monto recibido / Paga con" }, paymentChange: { candidates: movementPaymentChangeCandidates, value: input.payment?.isCash && input.payment.change !== undefined && input.payment.change !== null ? number(input.payment.change) : undefined, label: "Vuelto" }, paymentMethod: { candidates: movementPaymentMethodCandidates, value: input.payment?.method ? select(pickSelectOption(movementSchema, movementPaymentMethodCandidates, [input.payment.method, "Efectivo"])) : undefined, label: "Método de pago" },
  });
  if (input.cashRegisterId && !pickPropertyName(movementSchema, movementCashRegisterCandidates)) movementBuilt.warnings.push("Movimientos no tiene relación a Caja. Las ventas se guardan, pero no quedan asociadas al turno.");
  if (input.payment?.isCash && !pickPropertyName(movementSchema, movementPaymentReceivedCandidates) && !pickPropertyName(movementSchema, movementPaymentChangeCandidates)) movementBuilt.warnings.push("Movimientos no tiene Monto recibido/Vuelto; el vuelto se calculó pero no quedó guardado.");
